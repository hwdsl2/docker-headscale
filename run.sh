...
fetch_usage_asset() {
  ...
  asset=$1
  base_url=${HS_USAGE_BASE_URL:-https://github.com/hwdsl2/vpn-extras/releases/download/v1.1.0}
  base_url=${base_url%/}
...
report_usage_counts() {
  ...
  [ "${HS_DISABLE_USAGE_COUNTS:-0}" != "1" ] || return 0
  $data_mounted || return 0

  current_version="${IMAGE_FLAVOR:-unknown}-${IMAGE_VER:-unknown}"
  arch=$(usage_arch)
  state_file="$USAGE_STATE_DIR/headscale.version"
  ...
  if [ -n "$action" ]; then
    if fetch_usage_asset "vpn-v1-headscale-$action-$arch"; then
...
USAGE_STATE_DIR="/var/lib/headscale/.headscale-usage"
data_mounted=false
data_existing=false
if grep -q " /var/lib/headscale " /proc/mounts 2>/dev/null; then
  data_mounted=true
fi
if $data_mounted && find /var/lib/headscale -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null | grep -q .; then
  data_existing=true
fi
